...
/** Redondea cantidades de stock a 1 decimal para display (evita 2.5999999999999996). */
...
interface DashboardProps {
  ...
  /** true para role='rep' (Sprint 2.6): oculta revenue/profit/margen — un
   *  representante no debe ver costos ni utilidad, ni siquiera de sus propias
   *  ventas (revelaría el costo por unidad del catálogo). */
  hideFinancials?: boolean;
}

export const Dashboard: React.FC<DashboardProps> = ({ clients, orders, inventory, calcWeeks, hideFinancials }) => {
  // Pase móvil (bloque 2.3): tarjetas en 2 columnas y secciones apiladas en celular.
  ...
  return <div>
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : `repeat(${hideFinancials ? 3 : 5}, 1fr)`, gap: 10, marginBottom: 16 }}>
      {!hideFinancials && <Card title="Revenue" value={fmt(tRev)} color="#1B7340" />}
      {!hideFinancials && <Card title="Gross profit" value={fmt(gP)} sub={tRev > 0 ? `${Math.round(gP / tRev * 100)}% margin` : ""} color="#1B7340" />}
      <Card title="Cases sold" value={tCases} color="#1A5276" /><Card title="Pending $" value={fmt(pend)} color={pend > 0 ? "#C41E3A" : "#1B7340"} /><Card title="In stock" value={`${q(tStock)} cases`} color="#6C3483" />
    </div>
    {(stale.length > 0 || lowS.length > 0 || outS.length > 0) && <div style={{ background: "#FDF2E9", borderRadius: 8, padding: "12px 16px", marginBottom: 16, borderLeft: "4px solid #D35400" }}><div style={{ fontSize: 14, fontWeight: 700, color: "#D35400", marginBottom: 6 }}>Action needed</div>{outS.map(i => <div key={i.productId} style={{ fontSize: 12, padding: "2px 0", color: "#C41E3A" }}>OUT: <b>{i.p?.name}</b></div>)}{lowS.map(i => <div key={i.productId} style={{ fontSize: 12, padding: "2px 0", color: "#D35400" }}>LOW: <b>{i.p?.name}</b> — {q(i.stock)} left</div>)}{stale.slice(0, 5).map(c => {
    ...
    <div style={{ display: "grid", gridTemplateColumns: isMobile || hideFinancials ? "1fr" : "1fr 1fr", gap: 16, marginBottom: isMobile ? 16 : 0 }}>
      {!hideFinancials && <div><ST>Top clients by profit</ST>{cProf.slice(0, 6).map((c, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #f0f0f0", fontSize: 13 }}><div><b>{c.name}</b> <Badge text={c.tier} color={TIER_CLR[c.tier]} /></div><div><span style={{ color: "#1B7340", fontWeight: 700 }}>{fmt(c.prof)}</span><span style={{ color: "#999", marginLeft: 6 }}>{c.oc} ord</span></div></div>)}</div>}
      <div><ST>Product velocity <span style={{ fontSize: 11, fontWeight: 400, color: "#999" }}>({Math.round(weeks)}wk span)</span></ST>{pVel.slice(0, 8).map(p => <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #f0f0f0", fontSize: 12 }}><span>{p.name}</span><div style={{ display: "flex", gap: 10 }}><span>{p.sold} sold</span><span style={{ color: "#777" }}>{p.wr}/wk</span><span style={{ color: p.st === 0 ? "#C41E3A" : p.st <= LOW ? "#D35400" : "#1B7340", fontWeight: 600 }}>{q(p.st)} stock</span>{p.wk < 3 && p.wk > 0 && <Badge text={`${p.wk}wk left`} color="#C41E3A" />}</div></div>)}</div>
    </div>
    <ST>Recent orders</ST>{orders.slice(-6).reverse().map(o => { const cl = clients.find(c => c.id === o.clientId); const cost = o.items.reduce((a, it) => a + itemCost(it) * it.qty, 0); return <div key={o.id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #f0f0f0", fontSize: 13 }}><div><b>{cl?.name || "?"}</b> <span style={{ color: "#999" }}>{fmtD(o.date)}</span></div><div style={{ display: "flex", gap: 8, alignItems: "center" }}><b>{fmt(o.total)}</b>{!hideFinancials && <span style={{ color: "#1B7340", fontSize: 11 }}>+{fmt((o.total || 0) - cost)}</span>}<Badge text={o.status} color={ST_CLR[o.status]} /></div></div>; })}
  </div>;
};
